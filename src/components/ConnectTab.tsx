    ...
    <div className={compact ? '' : 'h-full overflow-y-auto scroll-smooth'}>
          ...
          <div className="space-y-2">
            {syncLog.map((l, i) => (
              <div key={i} className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full flex-shrink-0"
                  style={{ background: l.type === 'found' ? 'var(--green)' : l.type === 'processing' ? 'var(--accent2)' : l.type === 'error' ? 'var(--red)' : 'var(--text4)' }} />
                <p className="text-[12px]"
                  style={{ color: l.type === 'found' ? 'var(--green)' : l.type === 'processing' ? 'var(--accent2)' : l.type === 'error' ? 'var(--red)' : 'var(--text3)' }}>
                  {l.text}
                </p>
              </div>
            ))}
            {syncState === 'syncing' && (
              <div className="flex items-center gap-2 mt-1">
                ...
                <p className="text-[12px]" style={{ color: 'var(--accent2)' }}>Processing...</p>
              </div>
            )}
          </div>
        </div>
      ...
      {!hasAccounts && (
        <div className="mx-4 mb-4 p-5 rounded-2xl text-center"
          ...
          <h3 className="font-bold text-lg mb-2" style={{ color: 'var(--text)' }}>Connect your inbox</h3>
          ...
          {[
            ...
            <div key={s.n} className="flex gap-3 px-4 py-3.5" style={{ borderBottom: i < 2 ? '1px solid var(--border)' : 'none' }}>
              <div className="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                ...
                <span className="text-[12px] font-bold" style={{ color: 'var(--accent2)' }}>{s.n}</span>
              </div>
              <div>
                <p className="text-[14px] font-semibold" style={{ color: 'var(--text)' }}>{s.title}</p>
                <p className="text-[12px] mt-0.5" style={{ color: 'var(--text4)' }}>{s.sub}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <p className="text-center text-[11px] pb-4 mx-8" style={{ color: 'var(--text4)' }}>
      ...
      <div className="mx-4 mb-8 p-4 rounded-2xl flex items-center justify-between"
        ...
        <div className="min-w-0 flex-1 pr-3">
          <p className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text4)' }}>Signed in as</p>
          <p className="text-[13px] font-medium truncate mt-0.5" style={{ color: 'var(--text2)' }}>{user.email}</p>
        </div>
        <button
          onClick={onSignOut}
...
function AccountRow({ account, index, onRemove, onSync, isSyncing }: {
  ...
  const colors = ['var(--accent)', 'var(--green)', 'var(--gold)', 'var(--red)']
    ...
    <div className="flex items-center gap-3 p-4 rounded-2xl" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          {providerIcon}
          <p className="text-[13px] font-semibold truncate" style={{ color: 'var(--text)' }}>{account.email}</p>
        </div>
        <p className="text-[11px] mt-0.5" style={{ color: account.sync_status === 'synced' ? 'var(--green)' : 'var(--accent2)' }}>
